...
Eight production hooks, 1,149 test cases, zero dependencies, one-command install and uninstall.
...
```bash
npx github:bram-wq/agent-guardrails#v0.2.0 try 'git push origin main 2>&1 | tail -2'
npx github:bram-wq/agent-guardrails#v0.2.0 try 'yes for sure'
npx github:bram-wq/agent-guardrails#v0.2.0 demo
```
...
```bash
npx github:bram-wq/agent-guardrails#v0.2.0 init       # copies hooks/ into ./.claude/hooks/, merges settings.json (backup first)
npx github:bram-wq/agent-guardrails#v0.2.0 doctor     # each hook allows a benign event of its own type AND refuses its incident
npx github:bram-wq/agent-guardrails#v0.2.0 report     # runs / fires / rate per hook for THIS project, with the denominator
npx github:bram-wq/agent-guardrails#v0.2.0 uninstall  # removes only what init added; a hook you modified is kept and named
```
...
```bash
npx github:bram-wq/agent-guardrails#v0.2.0 new my-guard   # hooks/my-guard.mjs + hooks/my-guard.test.mjs, tests pre-written in pairs
```
